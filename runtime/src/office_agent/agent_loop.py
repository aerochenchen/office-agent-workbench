...
def _build_system_prompt(catalog: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "先调用 plan_create 制定工作计划，再按计划逐步执行；\n"
        "- 若已有进行中的工作计划（active plan），每轮开始前先 plan_get 读取当前计划，"
        "每轮只推进一个可执行步骤，完成后用 plan_update_step 更新进度；\n"
        "- 用户说「继续」「接着做」时，按已有工作计划推进，禁止无故重新 plan_create；\n"
        "- 调用 finish 时若计划尚有未完成步骤，须在摘要中说明剩余项，"
        "并提示用户可按工作计划继续；\n"
        "- 单文件校对、两版 docx 对照、单一公文排版等简单、单步任务，不强制建 Plan；\n"
        "- 业务交付成果仍写入 output/；Plan 仅记录任务进度与家务状态，"
        "不替代 output/ 中的正式产出。\n"
        "Skill 使用纪律：\n"
