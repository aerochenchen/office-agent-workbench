...
src/
  lib/
    ...
    types.ts           # 共享类型
  components/
    SessionList.tsx     # 左栏：会话列表 + 工作区选择（含浏览器模式粘贴路径）
    ChatPanel.tsx       # 中栏：对话 + 工具调用状态行
    SkillPanel.tsx      # 右栏：Skill 列表、启停、安装（导入仅 Tauri）
    SettingsModal.tsx   # 模型网关设置（api_base / api_key / model / allowed_hosts）
    WorkspaceTree.tsx   # 历史组件，当前 App 未挂载
...
浏览器模式（`--web`）没有原生选文件夹对话框；左栏底部可粘贴**工作区**绝对路径。Skill 导入需使用 Tauri 桌面端。
...
Windows NSIS `文书通_*-setup.exe`、macOS 内测 DMG（未公证）均含 Runtime sidecar：
...
- Skill 安装：仅 Tauri 桌面端可用「导入技能」→ 选文件夹或 zip/md；安装前预览名称/类型/权限。浏览器模式不支持 Skill 导入。
- `WorkspaceTree` 组件仍保留于仓库，但当前 UI 未挂载；左栏为 `SessionList`（会话 + 工作区路径）。
