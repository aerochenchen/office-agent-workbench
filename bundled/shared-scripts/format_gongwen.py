...
def set_default_font(doc, font_name='仿宋_GB2312', font_name_ascii='Times New Roman', size=Pt(16)):
    """设置文档默认字体为3号仿宋"""
...
def set_line_spacing(doc, line_spacing=28.95):
    """设置固定行距28.95pt（约等于每页22行）"""
    style = doc.styles['Normal']
...
def adjust_paragraph_font(para, font_name='仿宋_GB2312', size=Pt(16), bold=False, color=None):
...
def format_docx(file_path):
    ...
    for i, para in enumerate(doc.paragraphs):
        ...
        if '：' in text and i == body_start:
            ...
            adjust_paragraph_font(para, '仿宋_GB2312', Pt(16))
            body_start = i + 1
            continue
        ...
        if text.startswith('一、') or text.startswith('二、') or text.startswith('三、'):
            ...
            adjust_paragraph_font(para, '黑体', Pt(16))
            continue
        
        # 二级标题（（一）...）：楷体
        if text.startswith('（一）') or text.startswith('（二）'):
            para.paragraph_format.first_line_indent = Cm(0.74)
            adjust_paragraph_font(para, '楷体_GB2312', Pt(16))
            continue
